...
Scenario Outline: Nordic_Norway_Linear_All_Post_NO - Validate MAM Asset Info table entry for Composite View ID:  <COMPOSITEVIEWID>
...
Scenario Outline: Nordic_Norway_Linear_All_Post_NO - PROCESSING - Validate Wochit Mapping table entry for Aspect Ratio: <ASPECTRATIO> [wochitRenditionStatus: <RENDITIONSTATUS> - isRenditionMoved: <ISRENDITIONMOVED>]
...
Scenario Outline: Nordic_Norway_Linear_All_Post_NO - FINISHED - Validate Wochit Mapping table entry for Aspect Ratio: <ASPECTRATIO> [wochitRenditionStatus: <RENDITIONSTATUS> - isRenditionMoved: <ISRENDITIONMOVED>]
  # RUN ONLY IN E2E, DO NOT RUN IN REGRESSION
  * configure abortedStepsShouldPass = true
  * eval if (!TargetTag.contains('E2E')) {karate.abort()}
  # ---------
