Feature:  E2E-00054: Querying getFixtures with a combination of filters where team.id.eq(valid), league.id.eq(valid), status.eq(valid) and date.eq(valid)

# [ TEST CODE ]
# @Regression
# Scenario: E2E-00054: Querying getFixtures with a combination of filters where team.id.eq(valid), league.id.eq(valid), status.eq(valid) and date.eq(valid)
#     When def result = karate.callSingle('classpath:CIMBL/Tests/E2E-00054/E2E-00054.feature')
#     Then match result.testStatus == 'pass'

Background:
  * url CIMBLbaseUrl
  * def currentPath = 'classpath:CIMBL/Tests/E2E-00054/'
